    ...
    <label :for="id" class="flex items-start text-sm font-normal text-gray-700 cursor-pointer select-none dark:text-gray-400">
      <div class="relative">
        <input
          ...
          type="checkbox"
          :checked="modelValue"
          @change="(e: Event) => $emit('update:modelValue', (e.target as HTMLInputElement).checked)"
          class="sr-only"
        />
        <div
          :class="modelValue ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'"
